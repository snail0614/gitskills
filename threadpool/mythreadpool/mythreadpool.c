...
thread_revoke	*g_thread_revoke = NULL;			/*线程取消*/
...
void pool_revoke_thread(int revoke_thread_num){
	if(revoke_thread_num == 0){
		return;
	}
	g_thread_revoke->revoke_num = revoke_thread_num;
...
void *thread_run(void *arg){
	while(1){
		...
		while(g_thread_revoke->is_revoke == 0 && g_pool->queue_work_num == 0 && !g_pool->shutdown){
		...
		if(g_thread_revoke->is_revoke != 0 && g_thread_revoke->revoke_count < g_thread_revoke->revoke_num){
			...
			pthread_mutex_unlock(&(g_pool->thread_run_lock));
			printf("从队列删除成功, 线程 %u 退出,目前线程池中有%d个线程\n",pthread_self(),g_pool->thread_num);
			pthread_exit(NULL);
			continue;
		}
		/*回调函数，执行任务*/
		if(g_pool->queue_work_num != 0){	
		//	printf("线程 %u is starting to work \n",pthread_self());
			assert(g_pool->queue_work_num != 0);
			assert(g_pool->queue_work != NULL);
			g_pool->queue_work_num--;
